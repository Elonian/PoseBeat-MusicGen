...
CONFIG="${CONFIG:-configs/motion_audio_adapter.yaml}"
GPUS="${GPUS:-2}"
...
echo "[PoseBeat] Logs: logs/dmd_motion_adapter/train_motion_adapter.log plus rank logs when using torchrun"
...
if [[ "${GPUS}" -gt 1 ]]; then
  torchrun --standalone --nproc_per_node "${GPUS}" scripts/train_motion_adapter.py "${train_args[@]}" "$@"
else
  python scripts/train_motion_adapter.py "${train_args[@]}" "$@"
fi
